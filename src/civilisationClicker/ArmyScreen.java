...
public class ArmyScreen implements UnitListener, InvasionListener, BattleOverScreenListener{
	...
	static BattleList battleList = new BattleList();
	...
	List<BattleOverScreen> finishedBattles = new ArrayList<BattleOverScreen>();
	ArmyScreen() {
	...
	void updateLabels() {
		...
		for(Country country : CivilisationMainClass.playerList) 
	...
	static void initialiseValues() {
		...
		ARMYPANELSIZE = new Rectangle(0, ResourceBar.RESOURCEBARSIZE,
	...
	public void createFinishBattleScreen(List<Unit> attackerTroops,
			List<Unit> defenderTroops, int province, int attacker, int defender,
			boolean attackerVictor) {
		BattleOverScreen battleFinished = new BattleOverScreen(attackerTroops, defenderTroops, province, attacker, defender, attackerVictor);
	...
	@Override
	public void battleOver(BattleOverScreen screen) {
...
class InvasionMap extends ScaledMap{
