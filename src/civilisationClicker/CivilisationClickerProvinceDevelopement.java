...
public class CivilisationClickerProvinceDevelopement {
	...
	CivilisationClickerProvinceDevelopement(int ID, int province) {
		this.ID = ID;
		this.province = province;
		buildingCost = new double[CivilisationClickerDataBase.buildingList.get(ID - 1).buildingList.size()];
		buildingCount = new int[CivilisationClickerDataBase.buildingList.get(ID - 1).buildingList.size()];
		buildingPointsPerSecond = new int[CivilisationClickerDataBase.buildingList.get(ID - 1).buildingList.size()];
		int i = 0;
	...
	void clickButton() {
		int a = CivilisationClickerDataBase.mapList.get(CivilisationClickerDataBase.chosenMap).provinceList.get(province).owner;
	...
	double pointsPerSecond() {
		...
		int owner = CivilisationClickerMapScreen.provinceList.get(province).owner - 1;
		int i = 0;
		for (CivilisationClickerBuilding building : CivilisationClickerDataBase.buildingList.get(ID - 1).buildingList) {
			double modifier = 0;
			for (CivilisationClickerResearch research : CivilisationMainClass.playerList.get(owner).researchList.get(ID - 1).researchList) {
	...
	boolean buyBuilding(int building, int modifier) {
		int owner = CivilisationClickerDataBase.mapList.get(CivilisationClickerDataBase.chosenMap).provinceList.get(province).owner;
		...
		CivilisationClickerDataBase.mapList.get(CivilisationClickerDataBase.chosenMap).provinceList.get(province).calculateUnrest();
	...
	void checkBuildingStage() {
		...
		if (stage > CivilisationClickerSuperScreen.maxStages) {
			stage = CivilisationClickerSuperScreen.maxStages;
		}
		currentStage = stage;
	}
