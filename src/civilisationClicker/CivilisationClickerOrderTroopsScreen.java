...
public class CivilisationClickerOrderTroopsScreen extends PaintedPanel implements ActionListener, MouseListener, OrderUnitPanelListener{
	...
	CivilisationClickerOrderTroopsScreen(int playerID, int province, boolean isAttacker) {
	...
	void createUnitPanels() {
		initialiseArrays();
		int a = CivilisationClickerDataBase.unitList.size();
		for (int i=0; i<a; i++) {
...
interface OrderListener {
	void removeOrderScreen(CivilisationClickerOrderTroopsScreen screen);
}
...
class OrderUnitPanel implements ActionListener, MouseListener{
	...
	void createGraphics() {
		CivilisationClickerUnit unit = CivilisationClickerDataBase.unitList.get(ID);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == unitCountField) {
			...
			int count = CivilisationClickerMathFunctions.parseInt(field);
