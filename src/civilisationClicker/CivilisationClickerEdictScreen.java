...
public class CivilisationClickerEdictScreen implements ProvinceIconListener, EdictListener{
	...
	CivilisationClickerEdictScreen() {
		createEdictScreen();
	...
	void createEdictScreen() {
		...
		for (int i=0; i<CivilisationClickerDataBase.edictList.size(); i++) {
	...
	void updateLabels() {
		...
		if (selectedProvince > -1) {
			if (CivilisationClickerMapScreen.provinceList.get(selectedProvince).owner != CivilisationMainClass.playerID)
				selectedProvince = findNextProvince();
			CivilisationClickerProvince province = CivilisationClickerMapScreen.provinceList.get(selectedProvince);
	...
	static void createProvinceIcons() {
		provinceIcons = new ImageIcon[CivilisationClickerMapScreen.provinceList.size()];
		for (int i=0; i<CivilisationClickerMapScreen.provinceList.size(); i++) {
			...
			Dimension imageDimension = CivilisationClickerMathFunctions.getScaledDimension(provinceSize, containerSize);
...
class ProvinceIcon implements MouseListener{
	...
	void createVisuals() {
		Dimension panelSize = new Dimension(CivilisationClickerEdictScreen.PROVINCEICONWIDTH,
				CivilisationClickerEdictScreen.PROVINCEICONHEIGHT);
		...
		provinceLabel = new JLabel(CivilisationClickerEdictScreen.provinceIcons[province]);
...
class EdictPanel implements MouseListener{
	...
	void createVisuals() {
		...
		nameLabel = new JLabel(CivilisationClickerDataBase.edictList.get(edict).Name);
	...
	void updateLabels() {
		int count = CivilisationClickerMapScreen.provinceList.get
				(CivilisationClickerEdictScreen.selectedProvince).edictList.get(edict).Count;
		countLabel.setText(CivilisationClickerMathFunctions.withSuffix(count));
	}
	void addEdictListener(EdictListener listener) {
		listeners.add(listener);
	}
	void showCost() {
		CivilisationClickerEdict edict = CivilisationClickerMapScreen.provinceList
				.get(CivilisationClickerEdictScreen.selectedProvince).edictList.get(this.edict);
		int screen = CivilisationClickerSuperScreen.governmentPointPool - 1;
