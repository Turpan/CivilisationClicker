...
public class CivilisationClickerQuickBuy implements QuickBuySlotListener{
	...
	void changeScreenType(int screenType) {
		...
		if (CivilisationClickerMapScreen.selectedProvince > -1) addPanel();
	}
	void addPanel() {
	...
	void loadImages() {
		...
		backgroundImages = new ImageIcon[CivilisationClickerDataBase.screenTypes.size()];
		for (int i=0; i<backgroundImages.length; i++) {
			String screenType = CivilisationClickerDataBase.screenTypes.get(i);
	...
	public void buildingBought(int screenType, int building, int amount) {
		...
		CivilisationClickerProvinceDevelopement developement = CivilisationClickerMapScreen.getSelectedProvince().developementList.get(screenType);
...
class QuickBuyMain implements ScrollListener{
	...
	void createGraphics() {
		...
		mainPanel.setBounds(CivilisationClickerQuickBuy.BUILDINGSLOTBOUNDS);
		...
		int size = 0;
		for (int i=0; i<CivilisationClickerDataBase.buildingList.get(ID).buildingList.size(); i++) {
		...
		if (scroll = size > CivilisationClickerQuickBuy.BUILDINGSLOTBOUNDS.height) {
			String screenType = CivilisationClickerDataBase.screenTypes.get(ID);
			Dimension scrollBarSize = new Dimension(CivilisationClickerQuickBuy.SCROLLBARBOUNDS.width, CivilisationClickerQuickBuy.SCROLLBARBOUNDS.height);
			...
			scrollBar.setBounds(CivilisationClickerQuickBuy.SCROLLBARBOUNDS);
			scrollBar.setOpaque(false);
			scrollBar.createScrollBar(size, CivilisationClickerQuickBuy.BUILDINGSLOTBOUNDS.height);
...
class QuickBuySlot implements MouseListener, MouseWheelListener{
	...
	void createGraphics() {
		CivilisationClickerBuilding building = CivilisationClickerDataBase.buildingList.get(screenType).buildingList.get(ID);
		mainPanel = new PaintedPanel();
	...
	void updateLabels() {
		CivilisationClickerProvinceDevelopement developement = 
				CivilisationClickerMapScreen.getSelectedProvince().developementList.get(screenType);
		countLabel.setText(CivilisationClickerMathFunctions.withSuffix(developement.buildingCount[ID]));
		costLabel.setText(CivilisationClickerMathFunctions.withSuffix(developement.buildingCost[ID]));
	}
	...
	void showCost() {
		...
		CivilisationClickerProvince selectedProvince = CivilisationClickerMapScreen.getSelectedProvince();
		CivilisationClickerProvinceDevelopement developement = selectedProvince.developementList.get(screenType);
	...
	static void loadImages() {
		slotBackground = new ImageIcon[CivilisationClickerDataBase.screenTypes.size()];
