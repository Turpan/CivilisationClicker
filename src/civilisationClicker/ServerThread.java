...
public class ServerThread extends Thread{
	...
	void processColonisationCheckData(Scanner inputScanner) {
		...
		if (MapScreen.provinceList.get(province).owner == 0) {
			CivilisationMainClass.mapScreen.changeProvinceOwner(province, playerSlot + 2);
			MapScreen.provinceList.get(province).coloniseInProgress = true;
			MapScreen.provinceList.get(province).coloniseProgress = 0;
	...
	void processColoniseProgressData(Scanner inputScanner) {
		...
		MapScreen.provinceList.get(province).recieveColoniseProgressData(progress, (playerSlot));
		inputScanner.close();
	}
	...
	void processBuildingCountData(Scanner inputScanner) {
		...
		MapScreen.provinceList.get(province).setBuildingCount(ID, building, count, playerSlot + 2);
	}
