...
public class CivilisationClickerLobbyScreen implements ActionListener, MouseListener{
	...
	Color playerColour[];
	CivilisationClickerColourSelector colourSelector;
	CivilisationClickerProvinceSelector startSelector;
	public CivilisationClickerLobbyScreen(boolean host, int players, int map) {
		playerSlots = players;
		isHost = host;
		this.map = map;
		loadImages();
	...
	void createLobbyScreen() {
		...
		Dimension selectorSize = new Dimension(CivilisationClickerProvinceSelector.WIDTH, CivilisationClickerProvinceSelector.HEIGHT - 20);
		String mapname = CivilisationClickerDataBase.mapList.get(map).map;
		String directory = "data/province maps/";
		startSelector = new CivilisationClickerProvinceSelector(mapname, directory, playerSlots, selectorSize, map);
		...
		colourSelector = new CivilisationClickerColourSelector();
