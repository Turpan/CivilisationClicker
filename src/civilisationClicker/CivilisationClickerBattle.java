...
public class CivilisationClickerBattle {
	static final int BATTLEDURATION = 600;
	List<CivilisationClickerUnit> attackerTroops;
	List<CivilisationClickerUnit> defenderTroops;
	int attacker;
	int defender;
	int province;
	int tickCount;
	...
	CivilisationClickerBattle(List<CivilisationClickerUnit> attackerTroops, int province, int attacker, int defender) {
		this.attackerTroops = attackerTroops;
		this.province = province;
		this.attacker = attacker;
		this.defender = defender;
	}
	void defenderJoinedBattle(List<CivilisationClickerUnit> defenderTroops) {
	...
	void calculatePower() {
		...
		for (CivilisationClickerUnit unit : attackerTroops) attackerpower += (unit.Power * unit.Count);
		attackerpower = attackerpower * (attackerBonus + 1);
		attackerPointTotal = attackerpower;
		if (defenderJoined) {
			for (CivilisationClickerUnit unit : defenderTroops) defenderpower += (unit.Power * unit.Count);
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		CivilisationClickerBattle other = (CivilisationClickerBattle) obj;
